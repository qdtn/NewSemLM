...
#### for this one, we produce Word_LABEL, so only one output
    ...
    def fit_shuffer(self, X1,X2, Y,  X_valid1=None,  X_valid2=None,Y_valid=None ,process_data_func=None):
...
def trainSemLM21(train_texts, valid_texts, we_dict_path1=None, we_dict_path2=None, dep=1,hidden_size1=32, hidden_size2=32, batch_size=200, save_folder=".", model_name="adv", max_epochs=120, load_dt=True):
    ...
    Yv = [[fm1.fY.map[fm1.fY.getFeatureValue(x)] + 1 for x in XX] for XX in Yv]
    ...
    mdl =Model21(fm1.f.current_index + 1, fm2.f.current_index + 1,
    ...
    mdl.fit_shuffer(X1,X2, Y, X1v,X2v, Yv, process_data_func=func)
...
def get_verb_embeddings(mdl, fm1, fm2, ofn, embedding_layer=2, fn = None, vob = None):
    '''
    ...
    Y1 = [["EOS"] for i in range(len(X1))]
    Y2 = [["EOS"] for i in range(len(X1))]
...
def get_embeddings(mdl, fm1, fm2, ofn, embedding_layer=2, fn = None, vob = None):
    ...
    X1 = [ [v[0]] for v in vobs]
    X2 = [ [v[1]] for v in vobs]
...
def calculate_selectional_preferences(config_path, model_path, list_preds, vob, output ):
    ...
    mdl, fm1, fm2 = loadSemLM21 (model_path, cfg['train'], cfg['valid'], cfg['we_dict1'], cfg['we_dict2'], cfg['dep'],
                 cfg['hidden_size1'], cfg['hidden_size2'],cfg['batch_size'], cfg['save_folder'])

    for p in list_preds:
        scores  = get_probability_is_argument(mdl, fm1, fm2, p)

        process_probability(fm1, fm2, scores,  p, output + "/" + p + ".out.txt", vobs=vob[p])

def process_probability(fm1,fm2,  scores, pred, output, vobs=None, levels=3):
    f = open(output, "w")
    '''
    scores =[]
    ...
    candidates = vobs[pred]
    need_to_process_lst = []
    for c in candidates:
        for lbl in fm2.fY.map.keys():
        #for lbl in fm2.f.map:

            if  c + "_" + lbl in fm1.fY.map:
...
def get_probability_is_argument(mdl, fm1, fm2, predicate_words,  pos=3):
    '''
    ...
    for p in range(pos):
        ...
        if  p >1:
            n = 1
        rs, rs_scores,   X1, X2, my_scores = get_scores_all(mdl, fm1, fm2, X1,X2, X_new1, X_new2, num_select=n)
        print ("result for ",p)
        print (rs)
        print (rs_scores)
        print (X1)
